...
class FeatureExtraction(nn.Module):
    def __init__(self, input_nc, ngf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_dropout=False):     # #输入通道数input_nc、生成特征图的通道数ngf、卷积层的层数n_layers、归一化层的类型norm_layer和是否使用dropout的标志use_dropout
        super(FeatureExtraction, self).__init__()
        downconv = nn.Conv2d(input_nc, ngf, kernel_size=4, stride=2, padding=1)     #通过一个卷积层downconv对输入进行下采样，将输入通道数从input_nc减少到ngf，同时减小特征图的尺寸
        model = [downconv, nn.ReLU(True), norm_layer(ngf)]
        for i in range(n_layers):
            in_ngf = 2**i * ngf if 2**i * ngf < 512 else 512
            out_ngf = 2**(i+1) * ngf if 2**i * ngf < 512 else 512
            downconv = nn.Conv2d(in_ngf, out_ngf, kernel_size=4, stride=2, padding=1)   #通过一个卷积层downconv对输入进行下采样，将输入通道数从input_nc减少到ngf，同时减小特征图的尺寸
            model += [downconv, nn.ReLU(True)]
            model += [norm_layer(out_ngf)]      #在每个卷积层后面都跟随一个ReLU激活函数和一个归一化层
        model += [nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1), nn.ReLU(True)]
        model += [norm_layer(512)]
        model += [nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1), nn.ReLU(True)]       #最后添加两个卷积层和归一化层，将特征图的通道数固定为512，并使用ReLU激活函数对特征进行处理
...
#计算两个特征张量之间的相关性（组合两个特征为一个张量），相关性张量可以用于匹配关键点、估算光流
