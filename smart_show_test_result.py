...
def result_verbose():
    ...
    paths_dict = {'data/test/cloth' : 1,
                  ...
                  #'data/test/pose' : 0, # NA
                  'result/gmm_final-vtonhd.pth/test/warp-cloth' : 1,
                  'result/gmm_final-vtonhd.pth/test/warp-mask' : 1,
                  'result/tom_final-vtonhd.pth/test/try-on' : 0,
                  }
    ...
    for fn in tqdm(filelist):
        ...
        for path in paths_dict.keys():
            if i in [0,1,4,5]:
                ...
                fn = src
            imgname = os.path.join(path, fn + "_00.jpg".format(paths_dict[path]))
            if 'image-parse' in path:
                imgname = os.path.join(path, fn + "_00.png".format(paths_dict[path]))
            img = Image.open(imgname).convert('RGB')
            #img.show()
            paddingimgs.paste(img, (width*i, 0))
            i += 1
